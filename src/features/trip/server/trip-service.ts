...
async function recomputeDriverOrders(
  ...
  if (updates.length > 0) {
    await tripRepository.bulkAssignRides(updates);
  }
}
...
export async function getTripDashboard(tripId: string) {
  ...
  if (!trip) return null;
  const members = await tripRepository.listMembers(tripId);
...
export async function updateTrip(input: {
  ...
  const trip = await tripRepository.getTrip(input.tripId);
  if (!trip) throw new Error("trip_not_found");
  if (trip.created_by !== input.userId) throw new Error("forbidden");
  if (tripRepository.isTripClosed(trip)) throw new Error("trip_closed");

  await tripRepository.updateTrip(input.tripId, input.userId, {
...
export async function listTripsForUser(userId: string) {
  return tripRepository.listTripsForUser(userId);
}

export async function joinTrip(tripId: string, userId: string, nickname: string) {
  return tripRepository.joinTrip(tripId, userId, nickname);
}
...
export async function updateMyTripProfile(input: {
  ...
  const members = await tripRepository.listMembers(input.tripId);
  ...
  return tripRepository.updateMember(mine.id, input.updates);
}
...
export async function autoAssignTrip(input: {
  ...
  if (tripRepository.isTripClosed(dashboard.trip)) {
  ...
  if (
    ...
    await tripRepository.updateMember(currentMember.id, { is_free_agent: true });
  }

  await tripRepository.clearAutoAssignableAssignments(tripId);
  const members = await tripRepository.listMembers(tripId);
  ...
  if (result.assignments.length > 0) {
    await tripRepository.bulkAssignRides(result.assignments);
  }

  const membersAfterAssignments = await tripRepository.listMembers(tripId);
...
export async function setTripAssignment(input: {
  ...
  if (!isOrganizer && input.driverMemberId != null && !isSelfAction) {
    throw new Error("forbidden");
  }
  ...
  await tripRepository.assignRide(
    passenger.id,
    input.driverMemberId,
    null,
    isSelfAction ? false : undefined
  );
  const membersAfterUpdate = await tripRepository.listMembers(input.tripId);
...
export async function closeTrip(tripId: string, userId: string) {
  const result = await tripRepository.closeTrip(tripId, userId);
  if (!result.ok) {
...
export async function leaveTrip(tripId: string, userId: string) {
  ...
  await tripRepository.leaveTrip(tripId, userId);
}
...
export async function getTripMember(memberId: string) {
  return tripRepository.getMemberById(memberId);
}
